...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `vcd` with the `NAME` of your provider.
...
- binary: pulumi-resource-vcd
  ...
  ignore: []
  ldflags:
  - -X github.com/pulumi/pulumi-vcd/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-vcd/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-vcd_darwin_amd64_v1
│   └── pulumi-resource-vcd
├── pulumi-vcd_darwin_arm64
│   └── pulumi-resource-vcd
├── pulumi-vcd_linux_amd64_v1
│   └── pulumi-resource-vcd
├── pulumi-vcd_linux_arm64
│   └── pulumi-resource-vcd
├── pulumi-vcd_windows_amd64_v1
│   └── pulumi-resource-vcd.exe
└── pulumi-vcd_windows_arm64
    └── pulumi-resource-vcd.exe
```
