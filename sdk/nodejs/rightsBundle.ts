// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

export class RightsBundle extends pulumi.CustomResource {
    /**
     * Get an existing RightsBundle resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: RightsBundleState, opts?: pulumi.CustomResourceOptions): RightsBundle {
        return new RightsBundle(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'vcd:index/rightsBundle:RightsBundle';

    /**
     * Returns true if the given object is an instance of RightsBundle.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is RightsBundle {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === RightsBundle.__pulumiType;
    }

    /**
     * Key used for internationalization
     */
    public /*out*/ readonly bundleKey!: pulumi.Output<string>;
    /**
     * Rights bundle description
     */
    public readonly description!: pulumi.Output<string>;
    /**
     * Name of rights bundle.
     */
    public readonly name!: pulumi.Output<string>;
    /**
     * When true, publishes the rights bundle to all tenants
     */
    public readonly publishToAllTenants!: pulumi.Output<boolean>;
    /**
     * Whether this rights bundle is read-only
     */
    public /*out*/ readonly readOnly!: pulumi.Output<boolean>;
    /**
     * Set of rights assigned to this rights bundle
     */
    public readonly rights!: pulumi.Output<string[] | undefined>;
    /**
     * Set of tenants to which this rights bundle is published
     */
    public readonly tenants!: pulumi.Output<string[] | undefined>;

    /**
     * Create a RightsBundle resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: RightsBundleArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: RightsBundleArgs | RightsBundleState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as RightsBundleState | undefined;
            resourceInputs["bundleKey"] = state ? state.bundleKey : undefined;
            resourceInputs["description"] = state ? state.description : undefined;
            resourceInputs["name"] = state ? state.name : undefined;
            resourceInputs["publishToAllTenants"] = state ? state.publishToAllTenants : undefined;
            resourceInputs["readOnly"] = state ? state.readOnly : undefined;
            resourceInputs["rights"] = state ? state.rights : undefined;
            resourceInputs["tenants"] = state ? state.tenants : undefined;
        } else {
            const args = argsOrState as RightsBundleArgs | undefined;
            if ((!args || args.description === undefined) && !opts.urn) {
                throw new Error("Missing required property 'description'");
            }
            if ((!args || args.publishToAllTenants === undefined) && !opts.urn) {
                throw new Error("Missing required property 'publishToAllTenants'");
            }
            resourceInputs["description"] = args ? args.description : undefined;
            resourceInputs["name"] = args ? args.name : undefined;
            resourceInputs["publishToAllTenants"] = args ? args.publishToAllTenants : undefined;
            resourceInputs["rights"] = args ? args.rights : undefined;
            resourceInputs["tenants"] = args ? args.tenants : undefined;
            resourceInputs["bundleKey"] = undefined /*out*/;
            resourceInputs["readOnly"] = undefined /*out*/;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(RightsBundle.__pulumiType, name, resourceInputs, opts);
    }
}

/**
 * Input properties used for looking up and filtering RightsBundle resources.
 */
export interface RightsBundleState {
    /**
     * Key used for internationalization
     */
    bundleKey?: pulumi.Input<string>;
    /**
     * Rights bundle description
     */
    description?: pulumi.Input<string>;
    /**
     * Name of rights bundle.
     */
    name?: pulumi.Input<string>;
    /**
     * When true, publishes the rights bundle to all tenants
     */
    publishToAllTenants?: pulumi.Input<boolean>;
    /**
     * Whether this rights bundle is read-only
     */
    readOnly?: pulumi.Input<boolean>;
    /**
     * Set of rights assigned to this rights bundle
     */
    rights?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Set of tenants to which this rights bundle is published
     */
    tenants?: pulumi.Input<pulumi.Input<string>[]>;
}

/**
 * The set of arguments for constructing a RightsBundle resource.
 */
export interface RightsBundleArgs {
    /**
     * Rights bundle description
     */
    description: pulumi.Input<string>;
    /**
     * Name of rights bundle.
     */
    name?: pulumi.Input<string>;
    /**
     * When true, publishes the rights bundle to all tenants
     */
    publishToAllTenants: pulumi.Input<boolean>;
    /**
     * Set of rights assigned to this rights bundle
     */
    rights?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Set of tenants to which this rights bundle is published
     */
    tenants?: pulumi.Input<pulumi.Input<string>[]>;
}
